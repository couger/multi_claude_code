...
一个基于 Electron 的 CLI 会话管理工具，用于管理和监控多个 Claude Code CLI 会话。
...
- **ESC 键确认**：防止误操作，拦截 ESC 键并显示确认对话框
- **外部 Claude Code 检测**：检测工作目录是否被其他 Claude Code 进程占用
...
### 告警设置
- 在设置面板的"告警"标签页中配置告警规则
- 支持按告警级别过滤（严重/警告/信息/调试）
- 支持静默模式，仅显示数字角标
- 外部 Claude Code 检测：基础检测（路径覆盖）+ 深度检测（进程检查）
...
### IPC 通信
- 主进程和渲染进程通过预定义通道通信
- IPC 通道定义在 `src/main/constants.ts` 和 `src/shared/constants.ts` 中
- 使用类型安全的 IPC 处理函数
...
### v1.1.0 (2026-04-19)
- ESC 键拦截确认对话框，支持键盘导航（←→选择，Enter确认）
- 外部 Claude Code 检测（基础检测 + 深度检测）
- 系统托盘功能优化
- 窗口自动隐藏/恢复功能
- 多显示器支持，可选择主显示器
- 远程创建会话设置移至网络页面
